...
class MmapEditor:
    ...
    def write_slice(self, start: int, data: bytes) -> int:
        ...
        if end > len(self._mmap):
            raise ValueError(
                f"Write would exceed file size ({end} > {len(self._mmap)})"
            )
    ...
    def replace(
        self, old: bytes, new: bytes, start: int = 0, end: Optional[int] = None
    ) -> int:
