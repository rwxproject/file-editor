...
from typing import Union, Optional, Callable, Iterator, List, Pattern
...
class TextEditor(ContextAwareStreamEditor):
    ...
    def find_lines(self, pattern: Union[str, Pattern], case_sensitive: bool = True) -> Iterator[tuple[int, str]]:
    ...
    def replace_in_lines(self, search_pattern: Union[str, Pattern], 
                        replacement: str, max_replacements: Optional[int] = None) -> bool:
    ...
    def insert_lines(self, line_number: int, lines: List[str]) -> bool:
        ...
        try:
            with safe_edit_context(self.file_path) as safe_op:
                ...
                with open(self.file_path, 'r', encoding=self.encoding) as infile, \
                     open(temp_file, 'w', encoding=self.encoding) as outfile:
    ...
    def replace_lines(self, start_line: int, end_line: int, new_lines: List[str]) -> bool:
    ...
    def comment_lines(self, start_line: int, end_line: int, 
    ...
    def uncomment_lines(self, start_line: int, end_line: int,
                       comment_prefix: str = "# ") -> bool:
    ...
    def indent_lines(self, start_line: int, end_line: int, 
                    indent: str = "    ") -> bool:
    ...
    def dedent_lines(self, start_line: int, end_line: int,
                    dedent_amount: int = 4) -> bool:
    ...
    def extract_section(self, start_pattern: Union[str, Pattern], 
                       end_pattern: Union[str, Pattern],
                       include_markers: bool = False) -> List[str]:
...
class FastTextEditor(TextEditor):
    ...
    def replace_line_fast(self, line_number: int, new_content: str) -> bool:
        ...
        try:
            indexed_file = self._get_indexed_file()
            indexed_file.replace_line(line_number - 1, new_content)  # Convert to 0-based
