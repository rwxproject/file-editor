...
from typing import Union, Optional, Callable, Iterator, Any, Dict, List
...
class CSVEditor(StreamEditor):
    ...
    def __init__(self, file_path: Union[str, Path], delimiter: str = ',', 
                 quotechar: str = '"', encoding: str = 'utf-8'):
    ...
    def process_rows(self, row_transformer: Callable[[Dict[str, str]], Optional[Dict[str, str]]],
    ...
    def filter_rows(self, predicate: Callable[[Dict[str, str]], bool],
                   output_path: Optional[Union[str, Path]] = None) -> Optional[Path]:
        ...
        return self.process_rows(
            lambda row: row if predicate(row) else None,
            output_path
        )
        
    def update_column(self, column_name: str, value_func: Callable[[str], str],
                     output_path: Optional[Union[str, Path]] = None) -> Optional[Path]:
    ...
    def add_column(self, column_name: str, value_func: Callable[[Dict[str, str]], str],
                  output_path: Optional[Union[str, Path]] = None) -> Optional[Path]:
    ...
    def sort_by_column(self, column_name: str, reverse: bool = False,
                      key_func: Optional[Callable[[str], Any]] = None) -> bool:
        ...
        try:
            ...
            # Sort rows
            sort_key = (lambda row: key_func(row[column_name]) if key_func 
            ...
            with safe_edit_context(self.file_path) as safe_op:
                ...
                with open(temp_file, 'w', newline='', encoding=self.encoding) as outfile:
                    writer = csv.DictWriter(outfile, fieldnames=headers,
                                          delimiter=self.delimiter, quotechar=self.quotechar)
                    writer.writeheader()
    ...
    def get_column_stats(self, column_name: str) -> Dict[str, Any]:
        ...
        try:
            numeric_values = [float(v) for v in non_empty_values if v.replace('.', '').replace('-', '').isdigit()]
            if numeric_values:
                stats.update({
...
if HAS_PANDAS:
    class PandasCSVEditor(CSVEditor):
        ...
        def __init__(self, file_path: Union[str, Path], chunk_size: int = 10000, **pandas_kwargs):
        ...
        def process_chunks(self, chunk_processor: Callable[[pd.DataFrame], pd.DataFrame],
                          output_path: Optional[Union[str, Path]] = None) -> Optional[Path]:
            ...
            try:
                ...
                for chunk in pd.read_csv(self.file_path, chunksize=self.chunk_size, **self.pandas_kwargs):
        ...
        def apply_operations(self, operations: List[Callable[[pd.DataFrame], pd.DataFrame]],
                           output_path: Optional[Union[str, Path]] = None) -> Optional[Path]:
    ...
    class PandasCSVEditor(CSVEditor):
        def __init__(self, *args, **kwargs):
            raise ImportError("pandas is required for PandasCSVEditor. Install with: uv add pandas")