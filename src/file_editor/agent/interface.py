...
from ..core.safety import safe_edit_context, performance_monitor
from ..formats.markdown import MarkdownEditor
from ..formats.csv import CSVEditor
from ..formats.text import TextEditor, FastTextEditor
...
class AgentFileSystem:
    ...
    def _check_file_size(self, file_path: Path):
        ...
        if file_path.exists():
            ...
            if size > self.max_file_size:
                raise ValueError(f"File {file_path} ({size} bytes) exceeds maximum size ({self.max_file_size} bytes)")
    ...
    def read_full_file(self, file_path: str, max_lines: Optional[int] = None) -> Optional[str]:
    ...
    def modify_file_section(self, file_path: str, start_line: int, 
        ...
        try:
            ...
            if success:
                ...
                self._log_operation('failed_modify', file_path, 
    ...
    def search_in_file(self, file_path: str, pattern: str, 
                      case_sensitive: bool = True, max_results: int = 100) -> List[Tuple[int, str]]:
        ...
        try:
            ...
            self._log_operation('search', file_path, f"'{pattern}' -> {len(results)} results")
            return results
    ...
    def list_files(self, directory: str = "", pattern: str = "*") -> List[str]:
        ...
        try:
            ...
            files = []
            for path in full_path.glob(pattern):
                if path.is_file():
...
class SpecializedAgentEditors:
    ...
    def markdown_edit_section(self, file_path: str, section_title: str, 
        ...
        try:
            ...
            if success:
                self.agent_fs._log_operation('markdown_edit_section', file_path, section_title)
            else:
                self.agent_fs._log_operation('failed_markdown_edit_section', file_path, section_title)
    ...
    def csv_filter_rows(self, file_path: str, column: str, value: str,
        ...
        try:
            ...
            if result_path:
                ...
                self.agent_fs._log_operation('csv_filter', file_path, f"{column}={value}")
                return str(rel_path)
